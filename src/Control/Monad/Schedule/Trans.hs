...
instance (Ord diff, TimeDifference diff, Monad m, MonadSchedule m) => MonadSchedule (ScheduleT diff m) where
  ...
  schedule' actions = do
    SplitVec{splitLeft = frees, splitRight = running} <- lift $ schedule' $ runFreeT <$> actions
    case splitVec $ freeFToEither <$> frees of
      SplitVec{ splitLeft = values, splitRight = delayed} -> _

    where
      freeFToEither :: FreeF f a b -> Either a (f b)
      freeFToEither (Pure a) = Left a
      freeFToEither (Free fb) = Right fb
